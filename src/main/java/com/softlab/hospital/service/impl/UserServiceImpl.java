...
public class UserServiceImpl implements UserService {
    ...
    public List<Map<String, Object>> selectAll(String userId) throws HosExection{
        ...
        if (null != list) {
            for (Doctor list0 : list){
                ...
                List<String> set = wjMapper.selectFilesByPrimaryKey(list0.getSystemId());
                int i = 1;
                for(String s : set) {
                    map.put("file"+i, s);
                    i++;
    ...
    @Override
    public RestData selectWjById(long docId) throws HosExection {
        List<String> files = wjMapper.selectFilesByPrimaryKey(docId);
        if (null != files && 0 != files.size()) {
            return new RestData(files);
        } else {
            throw new HosExection("该医生没有文件");
        }
    }

    @Override
    public RestData exit(String userId) throws HosExection {
        if (0 < userMapper.updateTokenByuserId(userId)) {
            return new RestData(0, "success");
        } else {
            throw new HosExection("退出登录失败");
        }
    }
}
